...
def main():
    ...
    image_group = add_parser.add_mutually_exclusive_group()
    image_group.add_argument('--with-image', action='store_true', help='Generate post with AI image')
    image_group.add_argument('--no-image', action='store_true', help='Generate post without image')
    ...
    if args.command == 'add':
        ...
        with_image = True if args.with_image else False if args.no_image else None
        success = scheduler.add_post(args.topic, args.time, args.content, with_image)
